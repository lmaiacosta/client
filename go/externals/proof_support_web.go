...
func (t WebServiceType) NormalizeRemoteName(m libkb.MetaContext, s string) (ret string, err error) {
	...
	protocolAssertsHTTPS := len(prot) > 0 && prot == "https"
	proofTypeAssertsHTTPS := len(t.scheme) > 0 && t.scheme == "https"
...
func init() {
	externalServices.ManuallyRegister("web", WebServiceType{})
	externalServices.ManuallyRegister("http", WebServiceType{scheme: "http"})
	externalServices.ManuallyRegister("https", WebServiceType{scheme: "https"})
}
